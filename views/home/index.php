    ...
    <div class="layui-header">
        ...
        <ul class="layui-nav layui-layout-left">
            ...
            <?php foreach ($menus as $key => $menu) { ?>
                <li class="layui-nav-item layui-hide-xs <?php if ($key == 0) {
                    echo 'layui-this';
                } ?>" lay-unselect><a nav-bind="xt<?= $key ?>"><?= $menu['name'] ?></a></li>
            <?php } ?>
            <!-- 小屏幕下变为下拉形式 -->
        ...
        <ul class="layui-nav layui-layout-right">
            ...
            <li class="layui-nav-item" lay-unselect>
                ...
                <dl class="layui-nav-child">
                    ...
                    <hr>
                    <dd lay-unselect>
                        <a ew-event="logout" data-url="<?= \yii\helpers\Url::to(['home/logout']) ?>">退出</a>
                    </dd>
                </dl>
            </li>
            <li class="layui-nav-item" lay-unselect>
                <a ew-event="theme" data-url="<?= \yii\helpers\Url::to(['theme']) ?>" title="主题"><i
    ...
    <div class="layui-side">
        <div class="layui-side-scroll">
            ...
            <?php foreach ($menus as $key => $sys) { ?>
                <ul class="layui-nav layui-nav-tree <?php if ($key != 0) {
                    echo ' layui-hide';
                    ...
                    <?php
                        ...
                        <li class="layui-nav-item">
                            <a title="<?= \yii\helpers\Url::to([$menu['route']]) ?>" <?php if ($menu['route'] && empty($menu['items'])) {
                                echo 'lay-href="' . \yii\helpers\Url::to([$menu['route']]) . '"';
                            } ?>><i class="layui-icon layui-icon-home"></i>&emsp;<cite><?= $menu['name'] ?></cite></a>
                            <?php if (!empty($menu['items'])) { ?>
                                <dl class="layui-nav-child">
                                    <?php foreach ($menu['items'] as $btn) { ?>
                                        <dd><a title="<?= \yii\helpers\Url::to([$btn['route']]) ?>"
                                               lay-href="<?= \yii\helpers\Url::to([$btn['route']]) ?>"><?= $btn['name'] ?></a>
                                        </dd>
                                    <?php } ?>
                                </dl>
                            <?php } ?>
